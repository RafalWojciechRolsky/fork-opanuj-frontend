...
describe('Form validation', () => {
  ...
  test('should pass validation if all fields are valid', () => {
    const errors = formValidator('Jan', 'Kowalski', 33);
    expect(errors).toEqual([]);
  });

  test('should return an error if first name is too short', () => {
    const errors = formValidator('J', 'Kowalski', 33);
    expect(errors).toContain(
      'Name (first or last) must be at least 2 characters long'
    );
  });

  test('should return an error if last name is too short', () => {
    const errors = formValidator('Jan', 'K', 33);
    expect(errors).toContain(
      'Name (first or last) must be at least 2 characters long'
    );
  });
  ...
  test('should return an error if age is not a number', () => {
    const errors = formValidator('Jan', 'Kowalski', NaN);
